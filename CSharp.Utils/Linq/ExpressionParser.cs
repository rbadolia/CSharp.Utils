...
namespace CSharp.Utils.Linq
{
    internal class ExpressionParser
    {
        ...
        public ExpressionParser(IList<ParameterExpression> parameters, string expression, 
            IList<object> values)
        {
        ...
        private void ProcessParameters(IList<ParameterExpression> parameters)
        {
        ...
        private void ProcessValues(IList<object> values)
        {
...
#pragma warning restore 0219
        ...
        private Expression ParseLogicalOr()
        {
            ...
            while (_token.id == TokenId.DoubleBar || TokenIdentifierIs("or"))
            {
                ...
                CheckAndPromoteOperands(typeof (ILogicalSignatures), op.text, ref left, ref right, 
                    op.pos);
                   ...
                   _token.id == TokenId.Amphersand)
            {
                ...
                switch (op.id)
                {
                    case TokenId.Plus:
                        ...
                        CheckAndPromoteOperands(typeof (IAddSignatures), op.text, ref left, 
                            ref right, op.pos);
                        left = GenerateAdd(left, right);
                        break;

                    case TokenId.Minus:
                        CheckAndPromoteOperands(typeof (ISubtractSignatures), op.text, ref left, 
                            ref right, op.pos);
        ...
        private Expression GenerateConditional(Expression test, Expression expr1, Expression expr2, 
            int errorPos)
        {
        ...
        private Expression ParseLambdaInvocation(Expression lambda)
        {
            int errorPos = _token.pos;
            NextToken();
            Expression[] args = ParseArgumentList();
        ...
        private Expression ParseTypeAccess(Type type)
        {
            ...
            if (_token.id == TokenId.OpenParen)
            {
                ...
                switch (FindBestMethod(type.GetConstructors(), args, out method))
                {
                    ...
                    default:
                        throw ParseError(errorPos, Res.AmbiguousConstructorInvocation, 
                            GetTypeName(type));
                }
            }
                ...
                switch (FindMethod(type, id, instance == null, args, out mb))
                {
                    ...
                    case 1:
                        ...
                        if (!IsPredefinedType(method.DeclaringType))
                            throw ParseError(errorPos, Res.MethodsAreInaccessible, 
        ...
        private Expression ParseAggregate(Expression instance, Type elementType, string methodName, 
            int errorPos)
        {
            ...
            if (
                FindMethod(typeof (IEnumerableSignatures), methodName, false, args, out signature) !=
                1)
        ...
        private Expression[] ParseArgumentList()
        {
            ValidateToken(TokenId.OpenParen, Res.OpenParenExpected);
            NextToken();
            Expression[] args = _token.id != TokenId.CloseParen
                ? ParseArguments()
                : new Expression[0];
            ValidateToken(TokenId.CloseParen, Res.CloseParenOrCommaExpected);
            NextToken();
        ...
        private void CheckAndPromoteOperand(Type signatures, string opName, ref Expression expr, 
            int errorPos)
        {
            var args = new[] {expr};
            MethodBase method;
        ...
        private void CheckAndPromoteOperands(Type signatures, string opName, ref Expression left, 
        ...
        private Exception IncompatibleOperandsError(string opName, Expression left, Expression right, 
            int pos)
        {
        ...
        private MemberInfo FindPropertyOrField(Type type, string memberName, bool staticAccess)
        {
            ...
            {
                MemberInfo[] members = t.FindMembers(MemberTypes.Property | MemberTypes.Field, flags, 
        ...
        private int FindMethod(Type type, string methodName, bool staticAccess, 
        ...
        private int FindIndexer(Type type, IList<Expression> args, out MethodBase method)
        {
            foreach (Type t in SelfAndBaseTypes(type))
            {
        ...
        private int FindBestMethod(IEnumerable<MethodBase> methods, IList<Expression> args, 
            out MethodBase method)
        {
            ...
            if (applicable.Length == 1)
            {
                ...
                for (int i = 0; i < args.Count; i++) args[i] = md.Args[i];
        ...
        private bool IsApplicable(MethodData method, IList<Expression> args)
        {
        ...
        private Expression PromoteExpression(Expression expr, Type type, bool exact)
        {
            ...
            if (expression != null)
            {
                ConstantExpression ce = expression;
                if (ce == nullLiteral)
                {
                    if (!type.IsValueType || IsNullableType(type))
        ...
        private static bool IsBetterThan(IList<Expression> args, MethodData m1, MethodData m2)
        {
            bool better = false;
            for (int i = 0; i < args.Count; i++)
            {
        ...
        private Expression GenerateStaticMethodCall(string methodName, Expression left, 
            Expression right)
        {
            return Expression.Call(null, GetStaticMethod(methodName, left, right), 
                new[] {left, right});
        }
        ...
        {
            _textPos = pos;
            ch = _textPos < _textLen ? _text[_textPos] : '\0';
        }
        ...
        private void NextToken()
        {
            ...
            switch (ch)
            {
                ...
                case '\'':
                    ...
                    {
                        NextChar();
                        while (_textPos < _textLen && ch != quote) NextChar();
 ...
 while (char.IsDigit(ch));
                    ...
                    throw ParseError(_textPos, Res.InvalidCharacter, ch);
            }
        ...
        {
            return _token.id == TokenId.Identifier &&
                   string.Equals(id, _token.text, StringComparison.OrdinalIgnoreCase);
        }
        ...
        {
            if (!char.IsDigit(ch)) throw ParseError(_textPos, Res.DigitExpected);
        }
